...
def adaptive_clustering(data):
    ...
    agglomerative = AgglomerativeClustering(n_clusters=None, distance_threshold=1., affinity='precomputed', linkage='complete')
    labels_agg = agglomerative.fit_predict(distance_matrix)

    b = np.max(np.array(labels_agg))
    a = {}
    for i in range(b+1):
        a[i] = []
    for i in range(len(labels_agg)):
        a[labels_agg[i]].append(i)
    new_order = []
    group_num_list = []
    for i in range(b+1):
        new_order.extend(a[i])
        group_num_list.append(len(a[i]))
    return new_order, group_num_list
...
class Dataset_Custom(Dataset):
    ...
    def __read_data__(self):
        ...
        self.new_order, self.group_num_list = adaptive_clustering(data)
