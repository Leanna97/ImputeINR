...
@register('ImputeINR_hypo')
class ImputeINRHypo(nn.Module):

    def __init__(self, group, global_depth, group_depth, in_dim, out_dim, global_hidden_dim, group_hidden_dim, use_pe, pe_dim, out_bias=0, pe_sigma=1024, fourier_coef=100, group_num_list=None):
        super().__init__()
        self.degree = fourier_coef
        self.use_pe = use_pe
        self.pe_dim = pe_dim
        self.pe_sigma = pe_sigma
        if group_num_list != None:
            self.group = len(group_num_list)
            group = self.group
        else:
            self.group = group
                ...
                if group_num_list != None:
                    ...
                    if out_dim % group == 0:
                        ...
                        if j != group - 1:
                            ...
                            if (out_dim - (group - 1) * ((out_dim // group) + 1)) > 0:
                                a = out_dim - (group - 1) * ((out_dim // group) + 1)
                            else:
                                a = out_dim - (group - 1) * (out_dim // group)
